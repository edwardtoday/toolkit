...
class string_utils {
 public:
  static std::string hextostr(std::vector<unsigned char> const& hexval,
 ...
 public:
  ...
  bool begin() {
    ...
    if (!::CryptAcquireContext(
      ...
      m_lasterror = errorinfo_t(GetLastError(),
  ...
  std::string hexdigest(bool uppercase = false) const {
    return string_utils::hextostr(m_digest, uppercase);
  }
    ...
    if (file.is_open()) {
      ...
      if (filesize > 0) {
        if (mdx.begin()) {
          ...
          do {
            size_t left = (filesize - total) > sizeof(chunk) ? sizeof(chunk) :
                          (filesize - total);
